...
as well as the gravitational fractionation of the \delN~signal in polar firn 
...
However previous studies based on more physical principles such as the 
...
For Greenlandic drill sites, temperatures were up to $3 \,^\mathrm{o}$C higher than present day during the HCO \citep{Dorthe1998}.
...
This dependence of the diffusion length on temperature
...
The seasonal temperature variation affects the firn diffusion length nonlinearly due to the saturation vapor pressure's exponential
...
For the first case we consider relatively warm and humid conditions (case \textbf{A} hereafter)
...
For the second case we consider cold and dry conditions (case \textbf{B} hereafter)
...
It is therefore not possible to get a correct estimate of the firn diffusion for "too warm" drill sites.
...
\vspace*{2mm}
	\begin{center}
			...
			\caption{Diffusion length and density profiles (black) for case A (solid curves: $T_{\mathrm{sur}} = -29 \,^{\circ} \mathrm{C}$,
				 $A = 0.22 \;\mathrm{myr^{-1}\, w.\, eq.}$) and B (dashed lines: $T_{\mathrm{sur}} = -55 \,^{\circ} \mathrm{C}$,
				$A = 0.032 \;\mathrm{myr^{-1}\, w.\, eq.}$). 
...
This time series represent a case A scenario with an applied diffusion length of  $8.40 \, \mathrm{cm}$.
...
While performing temperature reconstructions, the input parameters to Eq. \ref{eq:T_reconstruction} 
($A$, $\rho_{co}$, $\rho_o$, surface pressure $P$, $\mathcal{S}$ and $\sigma_{ice}$) are drawn from a Gaussian distribution with a predefined uncertainty (Table \ref{tbl:model_unc}).
...
\begin{table}[H]
	...
	\begin{tabular}{c c c c c c c } 
		...
		\midrule
		Uncertainty &	$\pm 5 \% A_\mathrm{mean} $  &  $\pm20 \, \mathrm{kg m^{-3}}$  &  $\pm30 \, \mathrm{kg m^{-3}}$   &    $\pm 2 \% P_\mathrm{mean} $  &     $\pm 1 \% \mathcal{S}_\mathrm{mean} $ 	 & $\pm 5 \% \sigma_{ice_\mathrm{mean}} $ \\
		\bottomrule		
	\end{tabular}
\end{table}
...
%The precision of each reconstruction method is quantified by firn diffusion model has some uncertain
%For every subsection, a data-based diffusion length estimate is obtained.
%A temperature is reconstructed by using Eq. \ref{eq:T_reconstruction},
%where the input parameters in the firn diffusion model (Eq. \ref{eq.difflengthintegration}) varies for each iteration.
%The input accumulation rate, tortuosity (close-off density), surface density, surface pressure and the thinning
%are drawn from a Gaussian distribution with a predefined uncertainty (Table \ref{tbl:model_unc}). 
...
%
%The precision of each reconstruction method is quantified by performing a sensitivity test that is based on $N$ repetitions. 
%Assuming that every ice core section consists of $J$ \delOx ~ and \delD ~points, then a repetition is based on a 
...
%
%Uncertainties in the firn diffusion model are take that the accumulation rate, the tortuorsity (close-off density), the surface density, surface pressure and the thinning rate are parameters that 
...
the uncertainties related to the firn diffusion model (parameters $A$, $P$, $\rho_o$ and $\rho_{co}$ in Table \ref{tbl:model_unc}) are included. 
...
\begin{table*}[]
	...
	\begin{tabular}{r r r r r r r } 
		...
		Applied diffusion	&	Est. diffusion 	& Est. T  [$^o\mathrm{C}$]\\
		\midrule
		$\sigma_{18}$	&	 $5.82$	 $\mathrm{cm}$&	$ 5.84\pm 0.13$ $\mathrm{cm}$	 &  $  -55.2 \pm 0.9 $ 	&	$8.50$  $\mathrm{cm}$&	$ 8.50 \pm 0.21$ $\mathrm{cm}$	& $ -29.2 \pm 1.1 $\\ 
	
	
		$\sigma_{\mathrm{D}}$&	$5.22$ $\mathrm{cm}$	&$ 5.23 \pm 0.11 $ $\mathrm{cm}$	 &	$-55.2  \pm 0.9 $ &	$ 7.86$	 $\mathrm{cm}$&	$7.85 \pm 0.17 $	$\mathrm{cm}$& 	$ -29.3\pm 1.0 $\\ 
	
	
		$\sigma_{17}$&	$5.89$ $\mathrm{cm}$	&$5.98 \pm 0.10 $ $\mathrm{cm}$	 &	$ -55.0 \pm 0.9$ &	$ 8.60$	 $\mathrm{cm}$&	$8.56 \pm 0.11 $	$\mathrm{cm}$& 	$-29.3 \pm 0.9 $\\ 	
	
	
		${}^{18}\Delta\sigma^2$  I &$ 6.6	$ $\mathrm{cm^2}$&	$ 6.8\pm 1.4 $ $\mathrm{cm^2}$	& $ -55.1 \pm 2.3$   &	$10.5$  $\mathrm{cm^2}$	&	$10.8 \pm 2.5$ $\mathrm{cm^2}$	&$ -28.9 \pm 4.1 $\\
	
	
		${}^{18}\Delta\sigma^2$  II& $6.6$ $\mathrm{cm^2}$	&	$6.6 \pm 0.7$ $\mathrm{cm^2}$	& $ -55.3 \pm 1.5$	&	$ 10.5$ $\mathrm{cm^2}$	&$10.5 \pm 1.0 $	 $\mathrm{cm^2}$& $-28.9 \pm 1.8$\\
	
	
		${}^{18}\Delta\sigma^2$  III& $6.6$ $\mathrm{cm^2}$	&	$6.0 \pm 0.3$ $\mathrm{cm^2}$		&  $-56.4  \pm 1.1$	&	$10.5$	 $\mathrm{cm^2}$&	 $10.3 \pm 0.6 $  $\mathrm{cm^2}$& $-29.3 \pm 1.4 $\\
	
	
		${}^{17}\Delta\sigma^2$  I &$ 7.4	$ $\mathrm{cm^2}$&	$ 8.4 \pm 0.8 $ $\mathrm{cm^2}$	& $-54.0  \pm 1.4$   &	$12.2$  $\mathrm{cm^2}$	&	$ 11.7\pm 2.2 $ $\mathrm{cm^2}$	&$-30.0 \pm 3.3$\\
	
	
		${}^{17}\Delta\sigma^2$  II& $7.5$ $\mathrm{cm^2}$	&	$7.5 \pm 0.6$ $\mathrm{cm^2}$	& $-55.3  \pm 1.3 $	&	$12.2$ $\mathrm{cm^2}$	&$ 12.5\pm 0.8$	 $\mathrm{cm^2}$& $-28.7 \pm 1.5 $\\
	
	
		$\sigma^2_{18}/\sigma^2_{17} $&$	0.976$& $ 0.956\pm 0.028 $ 	& --------------	&$0.977$*	& 	$0.988 \pm 0.037$* 	& -------------- \\
	
	
		$\sigma^2_{18}/\sigma^2_\mathrm{D} $&$	1.24$& $1.24 \pm 0.04 $ 	& $-55.8 \pm 12.5$	&$1.17$	& 	$1.17 \pm 0.04$ 	& $-27.3 \pm 24.3$\\
	
	
		$\sigma^2_{17}/\sigma^2_\mathrm{D} $&$	1.27$& $ 1.30 \pm 0.03 $ 	& $  -62.5 \pm 7.5$	&$1.20$	& 	$1.19 \pm 0.04$ 	& $ -19.8 \pm 28 $\\
		\bottomrule		
	\end{tabular}
...
technique being the least precise of all three differential diffusion approaches ($s_{\bar{T}} \approx 2.8 \,{}^{\circ} \text{C}$). 
...
Both the linear fit and the correlation techniques yield precision estimates of $1.8 \,{}^{\circ} \text{C}$ and 
 $1.5 \,{}^{\circ} \text{C}$, respectively.
On the contrary, both the linear fit and the correlation techniques yield similar precise estimates that
are almost 
On the contrary the 
correlation technique appears to be the most precise of the three techniques with precisions 
that are comparable of those achieved with the single diffusion techniques ($s_{\bar{T}} \approx 1.3 \,{}^{\circ} \text{C}$). All 10 experiments 
