...
%We want to emphasize that the presented precisions do not represent the best (or worst) obtainable precision 
%of the diffusion-based temperature reconstruction techniques. 
%We perturbate the input parameters to the firn diffusion model (Table \ref{tbl:model_unc})
%in order to facilitate an optimal comparison between the single and the differential diffusion techniques.
%Especially for the Holocene period, parameters such as the close-off density
%and surface density are often assumed to be better constrained.
%Correspondingly, inaccurate estimates of the CFA system's diffusion length can 
%impact the precision of the reconstructions negatively (see Sec. \ref{sec:outlook}).
%When utilizing the diffusion techniques on long ice core records,
%we propose that the uncertainties of such model parameters should be selected
%based on specific characteristics of the ice core site and measurement system.
