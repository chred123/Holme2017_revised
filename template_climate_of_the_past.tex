...
%%%%% Changes equation/table/figure references to match the QSR preference in appendices
...
\newcommand{\delcorrel}{$\rho_{\delta{}^{18}\mathrm{O}/\delta\mathrm{D}}$}
...
\affil[1]{The Niels Bohr Institute, Centre for Ice and Climate, Juliane Maries Vej 30, 2100 Copenhagen, Denmark}
%\affil[]{ADDRESS}
...
that the most precise reconstructions are obtained when using the single isotope diffusion lengths, with precisions around $0.5\,^\mathrm{o}\mathrm{C}$. 
...
where the most precise method has a precision of $1.5\,^\mathrm{o}\mathrm{C}$. 
The diffusion length ratio method is the least precise with a precision of $11.8\,^\mathrm{o}\mathrm{C}$. 
...
From the combined analyses of the synthetic and ice core data, this study demonstrates that methods 
...
\label{Intro}
Polar precipitation stored for thousands of years at the ice caps of Greenland and Antarctica
contains invaluable information on past climatic conditions. The isotopic composition 
...
The use of the isotopic paleothermometer presents however some notable limitations.
The linear relationship between \delOx~and temperature commonly referred to as the ``spatial slope''
may hold for present conditions. However previous studies based on more physical principles as the 
borehole temperature reconstruction \citep{Cuffey1994, JOHNSEN1995a} 
as well as the thermal fractionation of the \delN~signal in polar firn 
\citep{Schwander1988, Severinghaus1998, Severinghaus1999} have
independently underlined the inaccuracy of the spatial isotope slope when it is extrapolated to past climatic 
conditions. Even though qualitatively the \delOx~signal comprises past temperature information, it fails
to provide a quantitative picture on the magnitudes of past climatic changes.


In a seminal paper in 2000, \cite{Johnsen2000} set the foundations for the quantitative description 
of the diffusive processes the water isotopic signal undergoes in the porous firn layer from the time
of deposition until pore close--off. Even though the main purpose of that work was to investigate
...
past firn temperatures. The temperature reconstruction method based on isotope firn diffusion requires 
data of high resolution. More specifically, if one would like to look into the differential diffusion signal, 
...
Nowadays, with the advent of commercial high--accuracy, high--precision Infra-Red spectrometers 
\citep{Crosson2008, Brand2009}, simultaneous measurements of \delOx~and \delD~have become 
easier to obtain. Coupling of these instruments to Continuous Flow Analysis systems \citep{Gkinis2011, Emanuelsson2015} 
...
In this work we attempt to look into all different flavors of temperature diffusion
...
due to the Dansgaard--Johnsen ice flow model overestimating  the past accumulation rates for the site. 
For some cases however this was not possible and approximately half of the datasets used here
have an age at approximately around Holocene Climate Optimum (\textbf{HCO} hereafter). 
Another interesting aspect of this study is that it uses water isotopic data sets of \delOx~and 
...
the various temperature reconstruction techniques that can possibly be employed for 
paleotemperature reconstructions.
The fundamentals of isotope diffusion theory are also presented. In order to avoid
significant overlap  with previously published works that have dealt with the matter
...
the water isotope signal from the time of deposition until  pore close--off. The process takes place in the
vapor phase and it can be described by Fick's second law as:
\begin{equation}
...
\label{eq.diffusion}
\end{equation}
where $z$ is the depth from surface, $\delta$ refers to the water isotope ratio signal,
$D \left( t \right)$ is the diffusivity coefficient and $\dot{\varepsilon}_z\left(t\right)$
the vertical strain rate.  The attenuation of the isotopic signal results in loss of information.
...
Since it is directly related to the diffusivity coefficient and the strain rate -the latter for the 
case of the firn being approximately proportional to the densification rate-, it can be regarded 
as a sensor of firn temperature as well as accumulation rate.  


The differential equation describing the evolution of $\sigma^2$ with time  can be 
...
\label{eq.strainrate}
\end{equation}
Then we can solve Eq. \ref{eq.diflength}  obtaining a solution for $\sigma^2$:
\begin{equation}
...
Both $\frac{\mathrm{d}\rho}{\mathrm{d}t}$  and $D\left(\rho\right)$ are quantities that are dependent on 
both the temperature and the accumulation. This dependence of the diffusion length on temperature
...
of water (\delOx, \delOxb, \delD) using boundary conditions characteristic of ice core sites from the Greenland
and the Antarctic Ice Cap.
For the first case we consider relatively warm and humid conditions (\textbf{WH} hereafter)
representative of Greenlandic ice coring sites
(e.g. GISP2, GRIP, NorthGRIP) with a surface temperature $T_{\mathrm{sur}} = -29 \,^{\circ} \mathrm{C}$ and 
annual accumulation $A = 0.22 \;\mathrm{myr^{-1}\, w.\, eq.}$
For the second case we consider relatively cold and dry conditions (\textbf{CD} hereafter)
...
annual accumulation $A = 0.032 \;\mathrm{myr^{-1}\, w.\, eq.}$
...
\vspace*{2mm}
	\begin{center}
			...
			\caption{Diffusion length and density profiles for the WH (solid lines) and CD (dashed lines) 
				scenarios for diffusion of the \delOx~(blue color), \delOxb~(purple color) and \delD~(red color) isotope signals. } 		
...
gradients within the lattice of the ice crystals. This process is orders of magnitude slower than 
firn diffusion. Several studies exist that deal with the estimate of the diffusivity 
...
temperature borehole profiles and assumed a steady state condition. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Reconstructing firn temperatures}
...
A fundamental property of the convolution operation is that it is equivalent to  multiplication
 in the frequency domain. The transfer function of the diffusion process will be given by the
...
\end{equation}%e10
In Eq. \ref{ftrans_gaussian}, $k = 2\pi / \Delta$ where $\Delta$ is the sampling resolution of the 
...
Wavelengths in the order of $50\,\mathrm{cm}$ and above remain largely unaltered while signals with 
wavelengths shorter than $20\,\mathrm{cm}$ are heavily attenuated. 


An estimate of the value of the 
diffusion length $\sigma$ can be obtained by looking at the power spectrum of the diffused 
isotopic time series. Assuming a measurement noise signal $\eta \left( k \right)$
apparent in the time series a model describing the power spectrum is given by: 
\begin{equation}
P_s =    P_0 {e}^{-k^2 \sigma^2} + {\vert \hat{\eta} \left( k \right) \vert} ^{2}. 
\label{eq:powerspectrum}
\end{equation}
We find that the noise signal is generally described well by autoregression of order 1 (AR-1).
The power spectral density of the latter may be written as \citep{Kay1981}:
\begin{equation}
|\hat{\eta}(k )|^2 = \frac{\sigma_{\mathrm{\eta}}^2 \Delta}
 {\left| 1+a_1 \exp{\left( -i k  \Delta \right) } \right|^2} {},
\end{equation}
where $a_1$ is the AR-1 coefficient and $\sigma_{\mathrm{\eta}}^2$ is the variance of the signal.


In Fig. \ref{fig:synthetic_power_spectra} we show an example of power spectra based on a synthetic time series. 
The time series is generated using an AR-1 process with coefficient $r_1$ and variance $\varepsilon$
\begin{equation}
\delta_{\mathrm{n}} - r_1 \delta_{\mathrm{n-1}} = \varepsilon_{\mathrm{n}}.
\label{eq:ar1}
\end{equation}
The time series is subsequently diffused with a $8.2\,\mathrm{cm}$ diffusion length Gaussian filter 
and sampled discretely at $2.5\,\mathrm{cm}$ resolution. 
We also added measurement noise equal to $0.09 \permil$ (1 standard deviation).
...
Using a least--squares approach we optimize the fit of $P_s$ to $\mathbb{P}_s$ by varying the four 
parameters $P_0$, $\sigma^2$, $a_1$ and $\sigma_{\mathrm{\eta}}^2$
The $\vert P_s - \mathbb{P}_s \vert^2$ least squares optimization resulted in a diffusion length of $8.5 \,\mathrm{cm}$.
...
In order to do this, one needs
to take into account three necessary corrections. 
...
filter specific for the CFA system \citep{Gkinis2011, Emanuelsson2015}. 
...
When an estimate of $\sigma^2_{\mathrm{firn}}$ is obtained the integrated firn column temperature can be calculated 
...
As it can be seen, the accuracy of the $\sigma^2_\text{firn}$ estimation and subsequently 
...
it can be a useful tool for assessing the accuracy of ice flow models. 
...
\begin{figure}[]	
	...
	\begin{center}		
		...
		\caption{PSD of a synthetic \delOx~time series plotted with respect to frequency (blue curve), 
			...
			the green curve represents the signal part of the fit
			and the magenta curve represents the noise part of the fit.	}
...
These spectral features are more likely to manifest in ice cores with boundary conditions that mitigate the effects
...
inspection of the spectrum. Annual peak corrections are included in the study of the ice core time series introduced in Sec.
\ref{sec:icecore_results}.


\begin{figure}[]
	\vspace*{2mm}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{Figure_5}
...
\end{equation}
As it can be seen in Fig. \ref{fig:diff_diffusion_example} the differential diffusion length signal is stronger for the case 
...
The evolution of IRMS techniques targeting the analysis of \delD~ 
\citep{BIGELEISEN1952, Vaughn1998, Gehre1996, Begley1997} in ice cores has allowed for dual isotopic records
...
(\delOx~and \delD~synthetic data generated as in section \ref{sec:single_diffusion}) an estimate of the
...
\end{equation}
Accurate estimates  of the thinning function however play a key role in the differential diffusion technique.
...
though in a small number of case spectral features in the low frequency area seem to have a strong 
...
can be based on the assumption that the initial precipitated isotopic signal presents a \Dxs~
that is invariable with time and as a consequence of this, the correlation signal \delcorrel~is expected
to have a maximum value at the time of deposition.
From the moment of  deposition, preferential diffusion of the \delOx~with respect to the 
\delD,~signal results in a decrease of the \delcorrel~ value, such that forward diffusing the \delD~signal 
with a Gaussian kernel of standard deviation equal to ${}^{18}\Delta\sigma^2$ will maximize 
the value of \delcorrel~\citep{vanderWel2015a}.  
...
acknowledging that the assumption that the \Dxs~signal is constant with time is most likely 
false -something that   certainly applies on  millennial time scales- and is thus likely to result in inaccuracies.
...
\begin{center}		
	...
	\caption{Differential diffusion lengths for the WH (solid lines) and CD 
		(dashed lines) scenarios for ${}^{18}\Delta\sigma^2$  (blue) and ${}^{17}\Delta\sigma^2$ (purple).
		The density profiles are represented with black.} 		
...
\begin{center}		
	...
	\caption{PSDs of \delOx~(blue) and \delD~(red) with respect to frequency where
		the inner subplot shows the $\ln \left({P_\mathrm{D}}/{P_{18}}\right)$ relation with respect to  $k^2$. 
		The ${}^{18}\Delta\sigma^2$ value is determined from the slope of the linear fit.
		The chosen cutoff frequency is marked by the vertical dashed line in the main plot.} 		
...
\begin{figure}[]	
	...
	\begin{center}		
		...
		\caption{Correlation coefficient between the \delOx~and the forward--diffused \delD~series 
		as a function of ${}^{18}\Delta\sigma^2$.  
		The synthetic data have been generated to simulated a CD scenario as in Sec. \ref{sec:single_diffusion}} 		
...
of the fractionation factors and carries no dependence to parameters related to the densification rates nor the 
...
\begin{figure}[]	
	...
	\begin{center}		
		...
		\caption{The diffusion length ratios $\sigma^2_{18}/\sigma^2_{\mathrm{D}}$ and $\sigma^2_{17}/\sigma^2_{\mathrm{D}}$
			...
			The $\sigma^2_{18}/\sigma^2_{17}$ is omitted due to its very low temperature sensitivity.} 		
...
quantities used for diffusion based temperature estimations. The tests consist of the generation of
synthetic data for \delOxb, \delOx~and \delD~and subsequent numerical diffusion with diffusion 
lengths as calculated for the WH and CD boundary conditions and presented in Fig. \ref{fig:diffusion_length_profiles}.
The time series are then sampled at a certain resolution $\Delta$ and measurement noise is added. Eventually,
estimates of diffusion lengths for all three isotopologues are obtained using the techniques we have described 
in the previous sections.

We start with the generation of \delOx~synthetic time series using the method outlined in Sec. \ref{sec:single_diffusion}.
For the AR-1 process we use $r_1 = 0.3$ and $\varepsilon = 120\,\permil^2$ for CD and $200\,\permil^2$ for WH. 
The data section is $20\, \mathrm{m}$ long and has an initial spacing of 
...
\end{align}
The isotopic series are then forward-diffused by means of numerical convolution 
with a Gaussian filter of variance $\sigma^2$
equal to the diffusion length for every case (Table \ref{tbl:synthetic_diff_lens}). 
Sampling with a discrete scheme of $\Delta = 0.025\, \mathrm{m}$ and 
addition of white measurement noise completes the process
of the time series generation. We use $0.05, \,0.07$ and $0.5\,\permil$ 
for \delOxb, \delOx~and \delD~respectively, with the numbers 
being representative of measurements uncertainties we have been 
observing over years of ice core measurements in our laboratory.
We repeat the process of time series generation 500 times. 
For each iteration we estimate the quantities $\sigma_{17}$, $\sigma_{18}$,
$\sigma_\mathrm{D}$, ${}^{17}\Delta\sigma^2$, ${}^{18}\Delta\sigma^2$ and 
the ratios $\sigma^2_{18}/\sigma^2_{\mathrm{D}}$,
$\sigma^2_{17}/\sigma^2_{\mathrm{D}}$ and $\sigma^2_{18}/\sigma^2_{17}$. 
...
For every value of the diffusion estimates we calculate a firn temperature.
Finally for the total of the 500 iterations we calculate a mean firn temperature $\overline{T}_{\text{firn}}$, 
a standard deviation $\sigma_{\bar{T}}$ and  a mean bias as: 
\begin{equation}
\text{MB} = \frac{1}{N} \sum_{i = 1}^{N} T_i - T_{\text{sur}} = \overline{T}_{\text{firn}} - T_{\text{sur}},
\end{equation}
...
surface temperature for the WH and CD scenarios.
...
The diffusion length ratio approach presents very large uncertainty bars and thus these results are not 
...
\begin{figure*}[]
	...
	\begin{center}
		...
		\caption{Mean biases for the single and differential diffusion.
			The error bars represent 1$\sigma_{\bar{T}}$}  \label{fig:synthetic_data_bias}
	\end{center}
\end{figure*}
...
\begin{table*}[]
	...
	\begin{tabular}{r r r r r r r } 
		...
		& &  CD & &  &WH&\\
		\midrule
		 &	Input diffusion  &	Output diffusion & Est. T [$^o\mathrm{C}$] &	
		Input diffusion	&	Output diffusion 	& Est. T  [$^o\mathrm{C}$]\\
		\midrule
		$\sigma_{18}$	&	 $5.73$	 $\mathrm{cm}$&	$ 5.81\pm 0.13$ $\mathrm{cm}$	 &  $  -54.9 \pm 0.5 $ 	&	$8.40$  $\mathrm{cm}$&	$ 8.43 \pm 0.20$ $\mathrm{cm}$	& $ -29.0 \pm 0.6 $\\ 
		$\sigma_{\mathrm{D}}$&	$5.13$ $\mathrm{cm}$	&$ 5.20 \pm 0.10 $ $\mathrm{cm}$	 &	$-55.0  \pm 0.4 $ &	$ 7.77$	 $\mathrm{cm}$&	$7.78 \pm 0.20 $	$\mathrm{cm}$& 	$ -29.1\pm 0.6 $\\ 
		$\sigma_{17}$&	$5.80$ $\mathrm{cm}$	&$5.94 \pm 0.12 $ $\mathrm{cm}$	 &	$ -54.7 \pm 0.4$ &	$ 8.50$	 $\mathrm{cm}$&	$8.61 \pm 0.16 $	$\mathrm{cm}$& 	$-28.8 \pm 0.5 $\\ 	
		${}^{18}\Delta\sigma^2$  I &$ 6.5	$ $\mathrm{cm^2}$&	$ 6.8\pm 1.3 $ $\mathrm{cm^2}$	& $ -54.6 \pm 2.3$   &	$10.2$  $\mathrm{cm^2}$	&	$10.6 \pm 2.2$ $\mathrm{cm^2}$	&$ -28.7 \pm 3.6 $\\
		${}^{18}\Delta\sigma^2$  II& $6.5$ $\mathrm{cm^2}$	&	$6.5 \pm 0.6$ $\mathrm{cm^2}$	& $ -54.8 \pm 1.1$	&	$ 10.2$ $\mathrm{cm^2}$	&$10.3 \pm 0.9 $	 $\mathrm{cm^2}$& $-28.7 \pm 1.2$\\
		${}^{18}\Delta\sigma^2$  III& $6.5$ $\mathrm{cm^2}$	&	$5.9 \pm 0.3$ $\mathrm{cm^2}$		&  $-56.0  \pm 0.5$	&	$10.2$	 $\mathrm{cm^2}$&	 $10.0 \pm0.5 $  $\mathrm{cm^2}$& $-29.4 \pm 0.9 $\\
		${}^{17}\Delta\sigma^2$  I &$ 7.3	$ $\mathrm{cm^2}$&	$ 8.3 \pm 1.2 $ $\mathrm{cm^2}$	& $-53.5  \pm 1.5$   &	$11.9$  $\mathrm{cm^2}$	&	$ 13.6\pm 1.6 $ $\mathrm{cm^2}$	&$-26.7 \pm 1.9$\\
		${}^{17}\Delta\sigma^2$  II& $7.3$ $\mathrm{cm^2}$	&	$7.4 \pm 0.6$ $\mathrm{cm^2}$	& $-54.9  \pm 0.9 $	&	$11.9$ $\mathrm{cm^2}$	&$ 12.2\pm 0.8$	 $\mathrm{cm^2}$& $-28.5 \pm 1.1 $\\
		$\sigma^2_{18}/\sigma^2_{17} $&$	0.975$& $ 0.957\pm 0.040 $ 	& --------------	&$0.977$*	& 	$0.959 \pm 0.027$* 	& -------------- \\
		$\sigma^2_{18}/\sigma^2_\mathrm{D} $&$	1.24$& $1.25 \pm 0.05 $ 	& $-56.9 \pm 12.9$	&$1.17$	& 	$1.18 \pm 0.04$ 	& $-30.7 \pm 16.0$\\
		$\sigma^2_{17}/\sigma^2_\mathrm{D} $&$	1.28$& $ 1.31 \pm 0.05 $ 	& $  -63.4 \pm 8.9$	&$1.20$	& 	$1.23 \pm 0.03$ 	& $ -40.0 \pm 9.9 $\\
		\bottomrule		
	\end{tabular}
	\caption{Simulations of a synthetic CD and WH scenario. The mean values
		and standard deviations of the output diffusion lengths are shown together
...
(discrete/CFA). The majority of the data sets was analysed using CRDS instrumentation. In particular the L1102i, L2120i and L2130i
...
corresponding to them. No reconstruction techniques involving \delOxb ~are presented here due to the lack of 
...
drawn from a uniform distribution. Eventually, for every reconstruction method and every ice core site, we
calculate a mean and a standard deviation for the diffusion estimate, as well as a mean and a standard 
deviation for the temperature ($T_{\text{sur}}$, $\sigma_{\bar{T}}$). We would like to point out 
...
The estimated temperatures for ice cores covering the late and early Holocene are 
...
\center
	\caption{Ice core data sections and the corresponding drill site characteristic. Sources of data: \citep{Steig2013}$^\mathrm{1}$, 
		\citep{Oerter2004}$^\mathrm{2}$. Drill site characteristic sources:		
		\citep{Fegyveresietal2011}$^\mathrm{a}$, \citep{Oerter2004,Veres2013}$^\mathrm{b}$, \citep{Watanabeetal2003}$^\mathrm{c}$,
			\citep{Lorius1979}$^\mathrm{d}$, \citep{Gkinis2014}$^\mathrm{e}$, \citep{Johnsen2000}$^\mathrm{f}$, \citep{Guillevic2013,Rasmussen2013}$^\mathrm{g}$.
		 }\label{tbl:drill_sites}
	\begin{tabular}{l c c c c c c c c c} 
		\toprule
		Site Name &Depth & Age  & Annual T  &
		 A & P & Thinning& Meas. & Analysis&$\Delta$\\
		 &[$\mathrm{m}$]&[$\mathrm{ka b2k}$]&[$^o\mathrm{C}$]& [$\mathrm{myr^{-1}}$]&
		...
		Dome F$^\mathrm{c}$& $302-307$& 9.6 & $-57.3$ &0.04 & 0.61  &  0.93 &$\delta$D, $\delta^{18}$O&CFA1102&0.5\\
		Dome C$^\mathrm{d}$ &$308-318$ & 9.9 &$-53.5 $ &0.04 & 0.65 &   0.93&$\delta$D, $\delta^{18}$O&IRMS&2.5\\
		GRIP early$^\mathrm{f}$&$1449-1466$ & 9.4 &$-31.6 $ & 0.23 & 0.65  &  0.42&$\delta$D, $\delta^{18}$O&2130&2.5\\
		NEEM dis$^\mathrm{g}$ &$1380-1392 $&10.9 &$-29.0$ & 0.22 &  0.72 &  0.31&$\delta$D, $\delta^{18}$O&2120&5.0\\       
		NEEM CFA$^\mathrm{g}$& $1382-1399$& 10.9 &$-29.0 $& 0.22 & 0.72  & 0.31&$\delta$D, $\delta^{18}$O&CFA1102&0.5\\			
...
\begin{figure*}[]
	...
	\begin{center}
		...
		\caption{Reconstructed temperatures from the $\sigma^2_{18}$ (blue circles), 
...
\center
	...
	\begin{tabular}{l r r r r r r} 
		...
		\midrule
		GRIP mid & 7.83 $\pm$ 0.15$\,\mathrm{cm}$& 7.20 $\pm$ 0.13$\,\mathrm{cm}$& 9.4 $\pm$ 0.9$\,\mathrm{cm}^2$& 9.6 $\pm$ 0.6$\,\mathrm{cm}^2$& 0.2$\pm$ 0.1$\,\mathrm{cm}^2$& 1.18$\pm$ 0.02\\
		& -32.9$\pm$ 0.5$\,^\mathrm{o}$C & -33.0$\pm$ 0.5$\,^\mathrm{o}$C& -32.7 $\pm$ 1.6$\,^\mathrm{o}$C& -32.3 $\pm$ 1.0$\,^\mathrm{o}$C& -82.0 $\pm$ 3.9$\,^\mathrm{o}$C& -34.2 $\pm$ 6.7$\,^\mathrm{o}$C\\
	
		GRIP late&8.52 $\pm$ 0.09$\,\mathrm{cm}$& 7.92 $\pm$ 0.14$\,\mathrm{cm}$& 9.9 $\pm$ 0.8$\,\mathrm{cm}^2$& 8.6 $\pm$ 0.6$\,\mathrm{cm}^2$& 4.8$\pm$ 0.5$\,\mathrm{cm}^2$& 1.16$\pm$ 0.02\\
		& -30.7 $\pm$ 0.3$\,^\mathrm{o}$C & -30.5 $\pm$ 0.5$\,^\mathrm{o}$C& -31.9 $\pm$ 1.3$\,^\mathrm{o}$C& -34.2 $\pm$ 0.9$\,^\mathrm{o}$C& -43.0 $\pm$ 1.4$\,^\mathrm{o}$C& -24.4 $\pm$ 7.7$\,^\mathrm{o}$C\\

		WAIS 2005A &6.95 $\pm$ 0.04$\,\mathrm{cm}$&--------------& --------------& --------------&--------------&--------------\\
		& -31.4 $\pm$ 0.1$\,^\mathrm{o}$C &-------------- &--------------&-------------- & --------------& --------------\\

		EDML &7.73 $\pm$ 0.04$\,\mathrm{cm}$& 7.13 $\pm$ 0.04$\,\mathrm{cm}$& 8.9 $\pm$ 0.3$\,\mathrm{cm}^2$& 8.2 $\pm$ 0.2$\,\mathrm{cm}^2$& 7.2$\pm$ 0.1$\,\mathrm{cm}^2$& 1.18$\pm$ 0.01\\
		& -42.8 $\pm$ 0.1$\,^\mathrm{o}$C & -42.5 $\pm$ 0.1$\,^\mathrm{o}$C& -44.6 $\pm$ 0.5$\,^\mathrm{o}$C& -45.9 $\pm$ 0.3$\,^\mathrm{o}$C& -47.7 $\pm$ 0.3$\,^\mathrm{o}$C& -32.2 $\pm$ 2.6$\,^\mathrm{o}$C\\
		
		NEEM&7.97 $\pm$ 0.20$\,\mathrm{cm}$& 7.20 $\pm$ 0.31$\,\mathrm{cm}$& 11.7 $\pm$ 1.5$\,\mathrm{cm}^2$& 10.2 $\pm$ 1.1$\,\mathrm{cm}^2$& 4.5$\pm$ 2.0$\,\mathrm{cm}^2$& 1.23$\pm$ 0.05\\
		& -31.9 $\pm$ 0.7$\,^\mathrm{o}$C & -32.5 $\pm$ 1.1$\,^\mathrm{o}$C& -28.5 $\pm$ 2.3$\,^\mathrm{o}$C& -30.8 $\pm$ 1.8$\,^\mathrm{o}$C& -45.8 $\pm$ 10.3$\,^\mathrm{o}$C& -49.2 $\pm$ 15.1$\,^\mathrm{o}$C\\
		
		NGRIP &8.65 $\pm$ 0.19 $\mathrm{cm}$&-------------- &--------------&-------------- & --------------&-------------- \\
		& -31.6 $\pm$ 0.6$\,^\mathrm{o}$C &--------------& --------------& --------------& --------------&--------------\\	
	
		Dome F&5.76 $\pm$ 0.15$\,\mathrm{cm}$& 4.92 $\pm$ 0.04$\,\mathrm{cm}$& 9.0 $\pm$ 1.8$\,\mathrm{cm}^2$& 5.4 $\pm$ 0.8$\,\mathrm{cm}^2$& 4.4$\pm$ 1.9$\,\mathrm{cm}^2$& 1.37$\pm$ 0.08\\
		& -57.6 $\pm$ 0.6$\,^\mathrm{o}$C & -58.5 $\pm$ 0.2$\,^\mathrm{o}$C& -54.2 $\pm$ 2.7$\,^\mathrm{o}$C& -60.4 $\pm$ 2.1$\,^\mathrm{o}$C& -63.7 $\pm$ 5.6$\,^\mathrm{o}$C& -80.7 $\pm$ 14.0$\,^\mathrm{o}$C\\
	
		Dome C &6.97 $\pm$ 0.14$\,\mathrm{cm}$& 6.34 $\pm$ 0.05$\,\mathrm{cm}$& 8.4 $\pm$ 1.9$\,\mathrm{cm}^2$& 6.7 $\pm$ 1.1$\,\mathrm{cm}^2$& 0.43$\pm$ 0.42$\,\mathrm{cm}^2$& 1.21$\pm$ 0.05\\
		& -52.8 $\pm$ 0.4$\,^\mathrm{o}$C & -52.5 $\pm$ 0.2$\,^\mathrm{o}$C& -54.4 $\pm$ 2.9$\,^\mathrm{o}$C& -56.9 $\pm$ 2.2$\,^\mathrm{o}$C& -88.8 $\pm$ 6.8$\,^\mathrm{o}$C& -42.1 $\pm$ 16.6$\,^\mathrm{o}$C\\
	
		GRIP early &9.32 $\pm$ 0.22$\,\mathrm{cm}$& 8.25 $\pm$ 0.03$\,\mathrm{cm}$& 18.7 $\pm$ 4.0$\,\mathrm{cm}^2$& 20.4 $\pm$ 1.9$\,\mathrm{cm}^2$& 6.6$\pm$ 1.1$\,\mathrm{cm}^2$& 1.27$\pm$ 0.06\\
		& -28.2 $\pm$ 0.7$\,^\mathrm{o}$C & -29.4 $\pm$ 0.1$\,^\mathrm{o}$C& -21.6 $\pm$ 4.2$\,^\mathrm{o}$C& -19.7 $\pm$ 1.6$\,^\mathrm{o}$C& -38.4 $\pm$ 2.5$\,^\mathrm{o}$C& -60.8 $\pm$ 16.4$\,^\mathrm{o}$C\\
	
		NEEM dis &10.33 $\pm$ 0.16$\,\mathrm{cm}$& 9.72 $\pm$ 0.19$\,\mathrm{cm}$& 12.1 $\pm$ 1.8$\,\mathrm{cm}^2$& 10.0 $\pm$ 0.8$\,\mathrm{cm}^2$& 1.6$\pm$ 0.2$\,\mathrm{cm}^2$& 1.13$\pm$ 0.02\\
		& -26.0 $\pm$ 0.4$\,^\mathrm{o}$C & -25.6 $\pm$ 0.5$\,^\mathrm{o}$C& -29.5 $\pm$ 2.4$\,^\mathrm{o}$C& -32.5 $\pm$ 1.3$\,^\mathrm{o}$C& -58.9 $\pm$ 1.5$\,^\mathrm{o}$C& -9.1 $\pm$ 11.5$\,^\mathrm{o}$C\\
	
		NEEM CFA&10.27 $\pm$ 0.15$\,\mathrm{cm}$& 9.65 $\pm$ 0.15$\,\mathrm{cm}$& 12.3 $\pm$ 1.0$\,\mathrm{cm}^2$& 11.4 $\pm$ 0.9$\,\mathrm{cm}^2$& 11.2$\pm$ 0.6$\,\mathrm{cm}^2$& 1.13$\pm$ 0.01\\
		& -26.2 $\pm$ 0.4$\,^\mathrm{o}$C & -25.8 $\pm$ 0.4$\,^\mathrm{o}$C& -29.1 $\pm$ 1.3$\,^\mathrm{o}$C& -30.3 $\pm$ 1.2$\,^\mathrm{o}$C& -30.6 $\pm$ 0.8$\,^\mathrm{o}$C & -12.1 $\pm$ 5.9$\,^\mathrm{o}$C\\
		
		NGRIP I&9.68 $\pm$ 0.13$\,\mathrm{cm}$& --------------&--------------& --------------&--------------&--------------\\
		& -29.1 $\pm$ 0.4$\,^\mathrm{o}$C & --------------&--------------&-------------- &--------------& --------------\\
		
		NGRIP II &10.14 $\pm$ 0.12$\,\mathrm{cm}$&-------------- &--------------&--------------&-------------- & --------------\\
		& -28.8 $\pm$ 0.3$\,^\mathrm{o}$C & --------------&-------------- &-------------- &-------------- &--------------\\
		\bottomrule	
	\end{tabular}
\end{table}
...
synthetic data, the following can be commented. Firstly, the three techniques based on the 
single isotope diffusion, perform similarly and of all the techniques
tested, yield the highest precision with a $\sigma_{\bar{T}} \approx 0.5 \,{}^{\circ} \text{C}$.
Additionally, the estimated temperatures $\overline{T}_{\text{firn}}$ are within 1$\sigma_{\bar{T}}$
...
inferior to single diffusion with  $\sigma_{\bar{T}} \approx 1.2 \,{}^{\circ} \text{C}$, with the subtraction 
technique being the least precise of all three differential diffusion approaches. 
...
On the contrary the 
correlation technique appears to be the most precise of the three techniques with precisions 
that are comparable of those achieved with the single diffusion techniques. All 10 experiments 
utilizing differential diffusion methods, yield an accuracy that lies within the 2$\sigma_{\bar{T}}$ range
(1$\sigma_{\bar{T}}$ range for 7 out of 10 experiments). We can conclude that experiments involving the estimation 
of the diffusion length ratio indicate that the latter are practically unusable due to very high uncertainties
with $\sigma_{\bar{T}}$ averaging to a value of  $\approx 12 \,{}^{\circ} \text{C}$ for all four experiments.
...
The precision $\sigma_{\bar{T}}$ of each reconstruction technique has been quantified
by averaging the standard deviations of the reconstructed temperatures (Table \ref{tbl:icecore_results}). 
Hence, the precision represents the uncertainty of the estimated diffusion signal, 
and is not an indication of the accuracy of the absolute temperature estimate.
...
The single diffusion methods have a $\sigma_{\bar{T}}$ of $0.5\,^\mathrm{o}\mathrm{C}$, while the differential diffusion methods ${}^{18}\Delta\sigma^2$ I, II and III
have a $\sigma_{\bar{T}}$ of $2.4\,^\mathrm{o}\mathrm{C}$,
$1.5\,^\mathrm{o}\mathrm{C}$ and $4.9\,^\mathrm{o}\mathrm{C}$, respectively.
...
was shown to be the most precise. 
...
a $\sigma_{\bar{T}}$ of $11.8\,^\mathrm{o}\mathrm{C}$. 
A similar precision was found from the synthetic data.


It is not possible to quantify the accuracy of the methods when applied on ice core data,
as the reconstructed temperatures represent the integrated firn column temperature.
The reconstructed temperatures should therefore not necessarily be identical to present day annual temperatures.
Instead, the root-mean-square deviation (RMSD) has been calculated in order to quantify the consistency between the
...
In theory, ${}^{18}\Delta\sigma^2$ should yield even more accurate temperature estimates as it
is independent of the systematic errors that can arise when correcting for ice and sampling diffusion. 
However, the RMSD between the temperature estimates of the ${}^{18}\Delta\sigma^2$ I and II techniques is found to be $3.8\,^\mathrm{o}\mathrm{C}$.  
Even higher RMSDs is found between the temperature estimates of the ${}^{18}\Delta\sigma^2$ I and III, and the ${}^{18}\Delta\sigma^2$ II and III techniques. Here, the RMSDs are found to be $24.8\,^\mathrm{o}\mathrm{C}$ and $23.8\,^\mathrm{o}\mathrm{C}$, respectively.
...
Finally, a high RMSD ($24.5\,^\mathrm{o}\mathrm{C}$) is also found when comparing the temperature estimates originating from the $\sigma^2_{18}$ and ${}^{18}\Delta\sigma^2$ III methods. 


The generally poor performance of the correlation-based method on ice core data contradicts the high accuracy and precision of the synthetic reconstructions, and is most likely caused by an oversimplification of the relationship between $\delta\mathrm{D}$ and $\delta^{18}\mathrm{O}$. The generation of the synthetic data is based on the assumption that $\delta\mathrm{D} = 8\cdot \delta^{18}\mathrm{O} + 10 \permil$. However, this premise excludes the time dependent \Dxs~signal. 
The correlation-based method can therefore be used to accurately reconstruct synthetic temperatures, while the accuracy and precision is much lower for ice core data,
...
The RMSDs between the temperature estimates of the $\sigma^2_{18}$ and ${}^{18}\Delta\sigma^2$ I methods, 
and the $\sigma^2_{18}$ and ${}^{18}\Delta\sigma^2$ II methods are found to be  
$3.9\,^\mathrm{o}\mathrm{C}$ and $4.8\,^\mathrm{o}\mathrm{C}$, respectively. This could therefore indicate that ${}^{18}\Delta\sigma^2$ I yields more accurate reconstructions than ${}^{18}\Delta\sigma^2$ II. 
However, ${}^{18}\Delta\sigma^2$ II has been shown to have the highest precision. It is therefore difficult to select the most accurate technique.
One should therefore not have a preferred technique without utilizing both methods 
on longer ice core sections. 
Basically, the reconstructed temperatures could be similar when the temperatures have been averaged over a longer record.

Longer ice core sections can provide the foundation for evaluating how well other parameters are constrained.
For instance, a wrong estimate of the thinning profile will scale the diffusion length estimates incorrectly 
and this will result in inaccurate temperature reconstructions.
High resolution measurements of ice cores spanning the complete Holocene and last glacial maximum would help evaluate how well the accumulation rate history of past climate is reconstructed. 
For the NGRIP ice core, a complete temperature profile has been reconstructed using the
$\sigma^2_{18}$ method \citep{Gkinis2014}, but the profile had a temperature gradient of around $10 \,\mathrm{^oC}$ between the
HCO and present day.  
The high gradient was found to be caused by an error in the thinning profile. 
This pointed to a necessary adjustment of the ice flow's total thinning function indicating that the
currently estimated accumulation rates were overestimated by 10\% at 8ky b2k.
This could be examined more thoroughly several places in Greenland and Antarctica 
by utilizing the single and differential diffusion techniques on ice cores
spanning both the Holocene and last glacial maximum.
...
$0.03^\mathrm{o}\mathrm{C}$ and $0.6^\mathrm{o}\mathrm{C}$.
...
The temperatures of the ${}^{18}\Delta\sigma^2$ techniques are more affected by the choice of parametrization,
with the temperature estimates of the ${}^{18}\Delta\sigma^2$ I, II and III techniques having RMSDs of 
$2.3^\mathrm{o}\mathrm{C}$, $2.3^\mathrm{o}\mathrm{C}$ and $2.5^\mathrm{o}\mathrm{C}$, respectively.
...
$0.9^\mathrm{o}\mathrm{C}$, $0.9^\mathrm{o}\mathrm{C}$ and $1.1^\mathrm{o}\mathrm{C}$, respectively.
In general, smaller RMSDs are found when comparing with the temperature estimates based on the \citet{Lamb2015} parametrization.
...
the ${\sigma^2_{18}}/{\sigma^2_\mathrm{D}}$ technique yields a RMSD of $5.4^\mathrm{o}\mathrm{C}$,
while the RMSD is $9.7^\mathrm{o}\mathrm{C}$ when comparing the results based on the parametrizations of
...
The precision of each technique was quantified by
...
From the combined analyses of the synthetic and ice core data, this study has demonstrated that methods 
based on the single isotope diffusion lengths result in the most accurate and precise estimates of past temperatures.	
...
We express the diffusivity as a function of firn density $\rho$ and we use:
\begin{equation}
...
\begin{equation}
2.1\cdot 10^{-5} {\left(\frac{T}{T_o}\right)}^{1.94} \left(\frac{P_o}{P}\right) 
...
\ln \left[\alpha_{\mathrm{Ice/Vapor}} \left( ^{17}\mathrm{O}/^{16} \mathrm{O} \right)\right] & = 0.529\ln\left[\alpha_{\mathrm{Ice/Vapor}} \left( ^{18}\mathrm{O}/^{16} \mathrm{O} \right)\right]
...
\label{eq:firn_tort}
\end{equation}
Based on Eq. (\ref{eq:firn_tort}), $\,\,\tau \rightarrow \infty$ for $\rho>804.3\,\mathrm{kgrm}^{-3}$
}
