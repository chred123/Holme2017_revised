...
From the combined analyses of the synthetic data and ice core data, this study demonstrates that methods 
...
\label{eq.diffusion}
\end{equation}
where $\delta$ refers to the water isotope ratio signal,
$D \left( t \right)$ is the diffusivity coefficient, $\dot{\varepsilon}_z\left(t\right)$
...
and it can therefore be regarded as a sensor of firn temperature as well as accumulation rate.
...
For the second case we consider cold and dry conditions (\textbf{CD} hereafter)
...
\vspace*{2mm}
	\begin{center}
			...
			\caption{Diffusion length and density profiles (black) for the WH (solid lines) and CD (dashed lines) 
...
This time series represent a WH scenario with an applied diffusion length of  $8.40 \, \mathrm{cm}$.
...
The \Dxs~signal is defined as the deviation from the metoric water line $\mathrm{d_{xs}}= \delta\mathrm{D}- 8\cdot\delta^{18}\mathrm{O} $ \citep{craig}.
...
\begin{figure}[]	
	...
	\begin{center}		
		...
		The synthetic data represent a CD scenario.} 		
...
\begin{figure}[]	
	...
	\begin{center}		
		...
		\caption{The diffusion length ratios $\sigma^2_{18}/\sigma^2_{\mathrm{D}}$ and $\sigma^2_{17}/\sigma^2_{\mathrm{D}}$
			...
			The $\sigma^2_{18}/\sigma^2_{17}$ is almost constant at $0.95$ and omitted here due to its very low temperature sensitivity.} 		
...
lengths as calculated for the WH and CD boundary conditions (as presented in Fig. \ref{fig:diffusion_length_profiles}).
...
\begin{table*}[]
	...
	\begin{tabular}{r r r r r r r } 
		...
		\bottomrule		
	\end{tabular}
	\caption{Simulations of a synthetic CD and WH scenario. The applied diffusion lengths are shown 
		together with the estimated diffusion lengths and the corresponding temperatures. }
...
deviation for the temperature. We would like to point out 
...
\center
	...
	\begin{tabular}{l c c c c c c} 
		...
		Dome C &6.97 $\pm$ 0.14$\,\mathrm{cm}$& 6.34 $\pm$ 0.05$\,\mathrm{cm}$& 8.4 $\pm$ 1.9$\,\mathrm{cm}^2$& 6.7 $\pm$ 1.1$\,\mathrm{cm}^2$& 0.4$\pm$ 0.4$\,\mathrm{cm}^2$& 1.21$\pm$ 0.05\\
...
tested, yield the highest precision with a $s_{\bar{T}} \approx 0.5 \,{}^{\circ} \text{C}$.
...
inferior to single diffusion with $s_{\bar{T}} \approx 1.2 \,{}^{\circ} \text{C}$, with the subtraction 
...
utilizing differential diffusion methods, yield an accuracy that lies within the 2$s_{\bar{T}}$ range
(1$s_{\bar{T}}$ range for 7 out of 10 experiments). We can conclude that experiments involving the estimation 
of the diffusion length ratio indicate that the latter are practically unusable due to very high uncertainties
with $s_{\bar{T}}$ averaging to a value of  $\approx 12 \,{}^{\circ} \text{C}$ for all four experiments.
...
The single diffusion methods have a $s_{\bar{T}}$ of $0.5\,^\mathrm{o}\mathrm{C}$, while the differential diffusion methods ${}^{18}\Delta\sigma^2$ I, II and III
have a $s_{\bar{T}}$ of $2.4\,^\mathrm{o}\mathrm{C}$,
...
similar high RMSDs are found between the temperature estimates of the ${}^{18}\Delta\sigma^2$ I and III, and the ${}^{18}\Delta\sigma^2$ II and III techniques. Here, the RMSDs are found to be $24.8\,^\mathrm{o}\mathrm{C}$ and $23.8\,^\mathrm{o}\mathrm{C}$, respectively.
...
with a Gaussian filter of variance $\sigma^2$ equal to the diffusion length 
...
For a WH scenario, the sampled outputs of the synthetic data before and after diffusion is shown in Fig. \ref{fig:appendix_synthetic_data}.
